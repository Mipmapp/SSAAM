...
const sendVerificationCode = async () => {
  ...
  const response = await fetch('ssaam-api.vercel.app/apis/students/send-verification', {
    method: 'POST',
    headers: { 
...
const handleNext = async () => {
  ...
  if (currentStep.value === 4) {
    ...
    try {
      const response = await fetch('ssaam-api.vercel.app/apis/students/verify-and-register', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': 'Bearer SSAAMStudents'
